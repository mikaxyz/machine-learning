...
import Html exposing (Html, button, footer, form, progress, section, text)
import Html.Attributes exposing (class, height, id, type_, width)
...
init id =
    ...
    ( { neuralNetwork = RemoteData.Loading
      ...
      --|> SceneOptions.toggle SceneOptions.showGridZOption
...
update { tagger } msg model =
    case msg of
                ...
                | scene =
                    model.scene
                        |> XYZScene.withCameraMap
                            (\camera ->
                                camera
                                    |> Camera.withOrbitY -(Vec2.getX pos / 200)
                                    |> Camera.withPositionMap
                                        (\position ->
                                            position
                                                |> Vec3.setY
                                                    (Vec3.getY position + (Vec2.getY pos / 2))
                                        )
                            )
              }
            , Cmd.none
        ...
        OnSubmit ->
            case model.neuralNetwork of
                RemoteData.Loaded neuralNetwork ->
                    ...
                    , Worker.Task.TrainNeuralNetwork 500 neuralNetwork
                        |> Worker.Task.start
                    )
        ...
        WorkerTaskProgress (Ok progress) ->
            ...
            in
            ( { model
                | neuralNetwork = RemoteData.Progress progress.complete progress.neuralNetwork
                , scene =
                    model.scene |> XYZScene.map (\_ -> Scene.treeWithConfig config.scene progress.neuralNetwork)
              }
            , Cmd.none
            )
...
view model =
    section []
        [ case ( model.defaultTexture, RemoteData.toMaybe model.neuralNetwork ) of
            ( Ok defaultTexture, Just _ ) ->
                WebGL.toHtml
